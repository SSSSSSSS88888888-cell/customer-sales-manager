...
const navigation = [
  ...
  { name: "目標設定", href: "/goals", icon: TrendingUp },
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <>
      <div className="flex items-center justify-between h-16 px-6 border-b border-black/30">
        ...
        {onClose && (
          <Button
            ...
            size="icon"
            onClick={onClose}
            className="md:hidden text-white/60 hover:text-white hover:bg-black/50"
          >
            <X className="h-5 w-5" />
          </Button>
        )}
            ...
            <Link
              ...
              className={cn(
                ...
                isActive
                  ...
                  : "text-white/60 hover:bg-black/50 hover:text-white"
              )}
            >
              <item.icon
                className={cn(
    ...
    <>
      ...
      {isOpen && (
        <>
          ...
          />
          {/* サイドバー */}
          <div className="fixed inset-y-0 left-0 w-60 bg-black z-50 md:hidden flex flex-col">
            {sidebarContent}
          </div>
        </>
      )}
